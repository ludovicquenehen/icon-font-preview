...
import { buildFontFaceCSS } from './fontRenderer';

export class IconBrowserPanel implements vscode.Disposable {
  public static currentPanel: IconBrowserPanel | undefined;
  private readonly _panel: vscode.WebviewPanel;
  private _disposables: vscode.Disposable[] = [];

  private constructor(panel: vscode.WebviewPanel, private store: IconStore) {
    this._panel = panel;
    this._render();
    ...
    store.onDidReload(() => {
      if (this._panel.visible) this._render();
    }, null, this._disposables);
  }

  static createOrShow(store: IconStore) {
  ...
  private _buildHtml(): string {
    const fontFaceCSS = this.store.fontFilePath
      ? buildFontFaceCSS(this.store.fontFamily, this.store.fontFilePath)
      : '';

    const iconsJson = JSON.stringify(
      this.store.icons.map(i => ({
        name: i.className,
        cp: i.codepoint,
        char: i.char,
      }))
